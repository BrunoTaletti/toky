...
from config import (
    UIConfig, TokenUtils, BASE_DIR,
    DataManager, ThemeManager, Translator,
    AppConfig
)

ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID('toky.2fa.manager.1')
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        icon = QIcon(resource_path("app-icon.ico"))
        self.setWindowIcon(icon)
        QApplication.setWindowIcon(icon)
        self.settings = AppConfig.load()
        self.current_theme = self.settings.get("theme", "dark")
        self.translator = Translator(self.settings.get("language", "en_US"))
        self.tokens = []
        self.setup_ui()
        self.load_tokens()
        self.apply_theme()
